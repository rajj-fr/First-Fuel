/* FIRST FUEL — cinematic trailer hero + ecommerce interactions */
...
if (heroTrailer) {
  ...
  const tryPlayTrailer = () => {
    const p = heroTrailer.play();
    if (p && typeof p.catch === "function") p.catch(() => {});
  };

  heroTrailer.addEventListener("loadeddata", tryPlayTrailer, { once: true });
  heroTrailer.addEventListener("canplay", tryPlayTrailer, { once: true });
  window.addEventListener("load", tryPlayTrailer, { once: true });
  setTimeout(tryPlayTrailer, 500);
}

// Hide the startup loader immediately; the trailer is independent of the old frame preload.
